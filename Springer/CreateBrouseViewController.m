...
@interface CreateBrouseViewController ()
{
...
@implementation CreateBrouseViewController
...
//    [self.view sendSubviewToBack:self.navigationController];
...
-(void)fnSetFontColor
{
    ...
    txtQuizNameInput.textColor = COLOR_BLACK;
...
//    customLeftBar.btnBack.hidden = NO;
    ...
    Title.LblTitle.text=@"Select Questions";
...
-(void)fnSetQuestionsCount
{
    ...
    if ([selectedFilter count] > 0) {
        [db fnGetQuestionList:arrChapter withUsed:used withunUsed:unused withBookmark:bookmark withAnswered:answered withNotes:notes];
...
-(void)fnSetFilteredQuestionsCount
{
    ...
    //if (tempdata.count > 0) {
    arrFilterType = [[NSArray alloc] initWithObjects:[NSNumber numberWithInt:unused], [NSNumber numberWithInt:used], [NSNumber numberWithInt:answered], [NSNumber numberWithInt:bookmark], [NSNumber numberWithInt:total], [NSNumber numberWithInt:note], nil];
    ...
    for (UILabel *lbl in arrCounterLabel) {
        switch (x) {
            ...
            case 5:
                lbl.text = [NSString stringWithFormat:@"%d", note];
                if (note == 0) {
                    lbl.text = @"";
                    lbl.backgroundColor = COLOR_CLEAR;
                }
...
-(void)fnSetCornerRadius
{
    ...
    scrollView.layer.shadowPath = [UIBezierPath bezierPathWithRect:CGRectMake(0, 5, scrollView.frame.size.width-2, scrollView.frame.size.height-2.5)].CGPath;
...
//
//   scrollView.layer.shadowPath = [UIBezierPath bezierPathWithRect:CGRectMake(scrollView.frame.origin.x, scrollView.frame.origin.y+scrollView.frame.size.height, scrollView.frame.size.width, scrollView.frame.size.height+10)].CGPath;
...
-(IBAction)onQuizTapped:(id)sender
{
    ...
    BrouseQuestionSetViewController *brouseQuestionSetViewController = [[BrouseQuestionSetViewController alloc] initWithNibName:@"BrouseQuestionSetViewController" bundle:nil];
    [self.navigationController pushViewController:brouseQuestionSetViewController animated:YES];
    [brouseQuestionSetViewController fnSetData:arrFilterQuestions AndTimer:YES AndCurrentQuestion:0 AndQuizScoreId:score_id AndIsSwipeGesture:YES];
...
-(void)fnRotateLandscape
{
    ...
    [scrollView setFrame:CGRectMake(99,50, 825, 523)];
    [lblQuestionFilter setFrame:CGRectMake(0, 0, 300, 21) ];
    ...
    [customButton_BnBegin setFrame:CGRectMake(751, 619, 176, 39)];
    [customButton_BnClear setFrame:CGRectMake(569, 619, 176, 39)];
